...
class Cache {
	...
	int get_data(int index, float[][] data, int len)
	{
		...
		if(more > 0)
		{
			...
			h.data = new_data;
			size -= more;
			do {int mid=h.len; h.len=len; len=mid;} while(false);
		}
	...
	void swap_index(int i, int j)
	{
		...
		do {float[] mid=head[i].data; head[i].data=head[j].data; head[j].data=mid;} while(false);
		do {int mid=head[i].len; head[i].len=head[j].len; head[j].len=mid;} while(false);
		if(head[i].len > 0) lru_insert(head[i]);
		if(head[j].len > 0) lru_insert(head[j]);

		if(i>j) do {int mid=i; i=j; j=mid;} while(false);
		for(head_t h = lru_head.next; h!=lru_head; h=h.next)
		{
			if(h.len > i)
			{
				if(h.len > j)
					do {float mid=h.data[i]; h.data[i]=h.data[j]; h.data[j]=mid;} while(false);
				else
				{
					// give up
					lru_delete(h);
...
abstract class Kernel extends QMatrix {
	...
	void swap_index(int i, int j)
	{
		do {svm_node[] mid=x[i]; x[i]=x[j]; x[j]=mid;} while(false);
		if(x_square != null) do {double mid=x_square[i]; x_square[i]=x_square[j]; x_square[j]=mid;} while(false);
	}
...
class Solver {
	...
	{
		Q.swap_index(i,j);
		do {byte mid=y[i]; y[i]=y[j]; y[j]=mid;} while(false);
		do {double mid=G[i]; G[i]=G[j]; G[j]=mid;} while(false);
		do {byte mid=alpha_status[i]; alpha_status[i]=alpha_status[j]; alpha_status[j]=mid;} while(false);
		do {double mid=alpha[i]; alpha[i]=alpha[j]; alpha[j]=mid;} while(false);
		do {double mid=p[i]; p[i]=p[j]; p[j]=mid;} while(false);
		do {int mid=active_set[i]; active_set[i]=active_set[j]; active_set[j]=mid;} while(false);
		do {double mid=G_bar[i]; G_bar[i]=G_bar[j]; G_bar[j]=mid;} while(false);
	}
...
class SVC_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		...
		do {byte mid=y[i]; y[i]=y[j]; y[j]=mid;} while(false);
		do {float mid=QD[i]; QD[i]=QD[j]; QD[j]=mid;} while(false);
	}
}
...
class SVR_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		do {byte mid=sign[i]; sign[i]=sign[j]; sign[j]=mid;} while(false);
		do {int mid=index[i]; index[i]=index[j]; index[j]=mid;} while(false);
		do {float mid=QD[i]; QD[i]=QD[j]; QD[j]=mid;} while(false);
	}
...
public class svm {
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for(i=0;i<prob.l;i++)
		{
			...
			do {int mid=perm[i]; perm[i]=perm[j]; perm[j]=mid;} while(false);
		}
		for(i=0;i<nr_fold;i++)
		{
		    ...
		    param.svm_type == svm_parameter.NU_SVC) && nr_fold < l)
		{
			...
			for (c=0; c<nr_class; c++)
				for(i=0;i<count[c];i++)
				{
					...
					do {int mid=index[start[c]+j]; index[start[c]+j]=index[start[c]+i]; index[start[c]+i]=mid;} while(false);
				}
			for(i=0;i<nr_fold;i++)
			{
				fold_count[i] = 0;
